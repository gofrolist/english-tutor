...
class TestQuestionModel:
    ...
    def test_question_creation_with_required_fields(self, db_session):
        ...
        assert question.sheets_row_id is not None
        assert question.task_id == task.sheets_row_id
    ...
    def test_question_correct_answer_validation(self, db_session):
        ...
        for correct_idx in range(len(answer_options)):
            question = Question(
                sheets_row_id=f"question_{correct_idx}",
                task_id=task.sheets_row_id,
    ...
    def test_question_sheets_row_id_ordering(self, db_session):
        """Test that questions are ordered by sheets_row_id."""
        task = Task(
        ...
        # Questions should be orderable by sheets_row_id
        questions = (
        ...
        assert questions[0].sheets_row_id < questions[1].sheets_row_id
